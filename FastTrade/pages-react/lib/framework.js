...
framework.internal.request = function (url, method, data, callback) {
    ...
    if (data !== null) {
        if (method === "GET" || method === "PUT" || method === "DELETE") {
            var str = framework.internal.toQueryString(data);
            if (str.length > 0) {
    ...
    request(method, url)
        ...
        .end(function (err, res) {
            ...
            if (errorCode === 200) {
                ...
                let urlMsg = url.substring(url.indexOf("qryFocusStatus"),url.length)
                if(urlMsg == "qryFocusStatus" && errorCode == 110){
